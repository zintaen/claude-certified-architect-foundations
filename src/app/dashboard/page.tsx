  ...
  return (
    <div className="flex-1 max-w-5xl mx-auto w-full p-6 md:p-12 flex flex-col gap-8">
      ...
      <div>
        ...
        <p className="text-foreground/60 mt-1">Review your past performance and track your growth.</p>
      </div>
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="glass-panel p-6 rounded-xl flex flex-col gap-2">
                <div className="text-4xl font-bold">{Math.floor((history?.averageTime || 0) / 60)}m</div>
              ...
              {history?.attempts?.map((entry: any, i: number) => (
                <motion.div 
                  ...
                  className={`flex items-center justify-between p-4 border-b border-white/5 last:border-0 ${
                    entry.score >= 700 ? 'border-l-4 border-l-green-500' : 'border-l-4 border-l-red-500'
                  }`}
                >
                  <div className="flex flex-col">
                    <div className="font-bold">Score: {entry.score}</div>
                    <div className="text-xs text-foreground/50">{new Date(entry.completed_at).toLocaleString()}</div>
                  </div>
