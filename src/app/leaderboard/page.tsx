        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="glass-panel rounded-2xl overflow-hidden">
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm ${
                      ...
                      i === 2 ? 'bg-[#cd7f32] text-black' : 'glass-panel'
                    }`}>
                      {i + 1}
                    </div>
                    <div>
                      <div className="font-bold">{entry.nickname || "Anonymous"}</div>
                      <div className="text-xs text-foreground/50">{new Date(entry.completed_at).toLocaleDateString()}</div>
                    </div>
                  </div>
              ...
              {(!stats?.topScores || stats.topScores.length === 0) && (
                <div className="p-8 text-center text-foreground/50">No scores recorded yet. Be the first!</div>
              )}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="glass-panel p-6 rounded-xl flex flex-col gap-2">
                <div className="text-4xl font-bold">{Math.floor((stats?.averageTime || 0) / 60)}m</div>
